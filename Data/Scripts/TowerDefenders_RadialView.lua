...
function RadialMenu:CreateSegment(title,icon,extraText)
    local newSection = { title = title, icon = icon, extraText = extraText }
...
function RadialMenu:_SectionClicked()
    if self.currentSelection then
        self:_FireEvent("OnSectionClicked",self.currentSelection,self.allSections[self.currentSelection])
...
function RadialMenu:_CheckSections()
    ...
    local segements = (360 / #self.SEGMENT_DATA)
