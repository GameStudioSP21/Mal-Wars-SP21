...
while true do
    ...
    if IS_FLIPED then
        Ease3D.EaseRotation(Leg01_swipe, rotation - Rotation.New(0,0,10), MOVEMENT_DURATION, Ease3D.EasingEquation.LINEAR)
    else
        Ease3D.EaseRotation(Leg01_swipe, rotation, MOVEMENT_DURATION, Ease3D.EasingEquation.LINEAR)
    end
    ...
    Task.Spawn(function()
        ...
        Task.Wait(MOVEMENT_DURATION/2)
        Ease3D.EaseRotation(Leg01_lift, leg01_lift_inital_rotation + Rotation.New(45,0,0), MOVEMENT_DURATION, Ease3D.EasingEquation.SINE)
    end)

    Task.Wait(MOVEMENT_DURATION)
    if IS_FLIPED then
        Ease3D.EaseRotation(Leg01_swipe, rotation + Rotation.New(0,0,50), MOVEMENT_DURATION, Ease3D.EasingEquation.LINEAR)
    else
        Ease3D.EaseRotation(Leg01_swipe, rotation - Rotation.New(0,0,50), MOVEMENT_DURATION, Ease3D.EasingEquation.LINEAR)
    end
