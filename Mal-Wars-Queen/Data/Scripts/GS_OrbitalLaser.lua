...
--local CoolEffect = script:GetCustomProperty("CoolEffect")
    ...
    if binding == FIRE_BIND  and not onCoolDown then
        if CheckView() then
            ...
            if(hitResult) then
                ...
                --CoolEffect:GetCustomProperty("sound01"):GetObject():Play()
                Task.Wait(COOL_DOWN_TIMER)
                -- timeAtFire = time()
                -- UpdateProgressBar()
                onCoolDown = false
                --CoolEffect:GetCustomProperty("sound02"):GetObject():Play()
            else
...
function DamageEnemies(hitResultPos)
	--CoolEffect:GetCustomProperty("sound03"):GetObject():Play()
...
function PlayAnimation(hitPos) 
    ...
    --CoolEffect:GetCustomProperty("sound04"):GetObject():Play()
