...
local STATE_WALK_TO_NODE = 9
local STATE_SWITCH_TO_NEXT_NODE = 10

local currentState = STATE_WALK_TO_NODE
local stateTime = 0
...
function SetState(newState)
    ...
    elseif newState == STATE_WALK_TO_NODE then
...
function Tick(deltaTime)
    ...
    if currentState == STATE_WALK_TO_NODE then
        ...
        if dist < 80 then
            SetState(STATE_SWITCH_TO_NEXT_NODE)
        end
